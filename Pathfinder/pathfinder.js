...
//       });
  ...
  createGrid() {
    for (let i = 0; i < 25; i++) {
      ...
      for (let j = 0; j < 25; j++) {
        gameState.grid[i].push(
          this.add.rectangle(i * 20 + 10, j * 20 + 10, 18, 18, 0x3399ff)
        );
  ...
  algorithm(grid, start, end) {
    ...
    grid.forEach((row) => {
      row.forEach((spot) => {
        gScore[spot.coordString] = Infinity;
      });
    });
    gScore[start.coordString] = 0;
    ...
    while (openSet.length()) {
      ...
      openSetHash.splice(openSetHash.indexOf(currentNode.coordString));
      console.log(currentNode.coordString);
      if (currentNode === end) {
      ...
      currentNode.neighbors.forEach((neighbor) => {
        ...
        if (tempGScore < gScore[neighbor.coordString]) {
          ...
          fScore[neighbor.coordString] =
            tempGScore + this.h(neighbor.getPos(), end.getPos());
...
class Spot {
  constructor(row, col, width, totalRows) {
    ...
    this.x = row * width;
    this.y = col * width;
    this.color = colorPanel.WHITE;
    this.neighbors = [];
    this.width = width;
    this.totalRows = totalRows;
    this.coordString = `${this.row};${this.col}`;
    this.id = this.coordString;
  }
  ...
  isBarrier() {
    return this.color === colorPanel.BLACK;
  }

  isStart() {
    return this.color === colorPanel.ORANGE;
  }

  isEnd() {
    return this.color === colorPanel.TURQUOISE;
  }
  ...
  makeClosed() {
    this.color = colorPanel.RED;
    gameState.grid[this.row][this.col].fillColor = colorPanel.RED;
  }

  makeOpen() {
    this.color = colorPanel.GREEN;
    gameState.grid[this.row][this.col].fillColor = colorPanel.GREEN;
  }
  ...
  makeBarrier() {
    ...
    this.color = colorPanel.BLACK;
    gameState.grid[this.row][this.col].fillColor = colorPanel.BLACK;
  }

  makeStart() {
    if (!this.color === colorPanel.WHITE) {
      return false;
    }
    this.color = colorPanel.ORANGE;
    gameState.grid[this.row][this.col].fillColor = colorPanel.ORANGE;
  }
  ...
  makeEnd() {
    ...
    this.color = colorPanel.TURQUOISE;
    gameState.grid[this.row][this.col].fillColor = colorPanel.TURQUOISE;
    return true;
  }

  makePath() {
    this.color = colorPanel.PURPLE;
    gameState.grid[this.row][this.col].fillColor = colorPanel.PURPLE;
  }
  ...
  updateNeighbors(grid) {
    ...
    if (
      ...
      !grid[this.row + 1][this.col].isBarrier()
    ) {
    ...
    //RIGHT
    if (
      this.col < this.totalRows - 1 &&
      !grid[this.row][this.col + 1].isBarrier()
    ) {
      this.neighbors.push(grid[this.row][this.col + 1]);
    }
  }
